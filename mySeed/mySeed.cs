...
namespace mySeed
{
    public partial class MySeed : Form
    {

        private readonly string[] bip39Wordlist =
        {
        ...
        private void TextBox_TextChanged(object sender, EventArgs e)
        {
            ...
            for (int i = 13; i < 25; i++)
            {
                ...
                if (txtBox?.Text != "") w24 = true;
            }

            BitArray code = new(w24 ? 264 : 132, false);

            for (int word = 0; word < (w24 ? 24 : 12); word++)
            {
                bool found = false;
                TextBox? txtBox = this.Controls["textBox" + (word + 1).ToString()] as TextBox;

                for (int i = 0; i < 2048; i++)
                {
                    if (txtBox?.Text == bip39Wordlist[i])
                    {
                        //BitArray val = new BitArray(new int[] { i });
                        int bitStart = word * 11;
                        for (int bitPos = 0; bitPos < 11; bitPos++)
                        {
                            code[bitStart + bitPos] = ((i >> (10 - bitPos)) & 1) == 1;
                        }
                        found = true;
                        break;
                    }
            ...
            code.CopyTo(bCode, 0);
            textBox_Code.Text = BitConverter.ToString(bCode).Replace("-", "");
            TextBox_Password_TextChanged(sender, e);

        }

        private void TextBox_Password_TextChanged(object sender, EventArgs e)
        {
            textBox_YourCode.Text = StringCipher.EncryptString(textBox_Code.Text, textBox_Password.Text);
        }

        private void TextBox_YourCode_TextChanged(object sender, EventArgs e)
        {
            textBox_PlainRestore.Text = StringCipher.DecryptString(textBox_YourCode.Text, textBox_Password.Text);
        }

        private void TextBox_PlainRestore_TextChanged(object sender, EventArgs e)
        {
            byte[] bCode = Enumerable.Range(0, textBox_PlainRestore.Text.Length)
                     .Where(x => x % 2 == 0)
                     .Select(x => Convert.ToByte(textBox_PlainRestore.Text.Substring(x, 2), 16))
                     .ToArray();
            BitArray code = new(bCode);

            for (int word = 0; word < 24; word++)
            {
                if (this.Controls["textBox_Out" + (word + 1).ToString()] is not TextBox txtBox) continue;
                if (code.Length > word * 11 + 10)
                {
                    int val = 0;
                    for (int i = 0; i < 11; i++)
                    {
                        if (code[word * 11 + i]) val += 1 << (10 - i);
                    }

                    txtBox.Text = val < 2048 ? bip39Wordlist[val] : "";
                }
                else
                {
                    txtBox.Text = "";
